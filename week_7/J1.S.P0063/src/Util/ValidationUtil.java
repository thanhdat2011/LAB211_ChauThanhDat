...
public class ValidationUtil {
    ...
    public double inputForParseDouble(String field) {
        ...
        while (!isDouble(res) || Double.parseDouble(res) <= 0) {
            res = inputWithOutEmpty("Input a positive number greater than 0! Please input again:");
        }
...
//    Validation for input Double
    ...
    // Validation for input Float
    
    public float inputFloatInRange(String field, double low, double high) {
        String res = inputWithOutEmpty(field);
        while (!isFloat(res) || Float.parseFloat(res) < low || Float.parseFloat(res) > high){
            if (!isFloat(res)) {
                System.out.println("Pls input number !!! Please input again");
            } else {
                System.out.println("Must be from " + low + " to " + high +" !!!");
    ...
    public float inputFloatGreaterThan(String field, double low) {
        String res = inputWithOutEmpty(field);
        while (!isFloat(res) || Float.parseFloat(res) < low){
            if (!isDouble(res)) {
                System.out.println("Pls input number !!! Please input again");            
            } else {
                System.out.println("Must be greater than " + low + " !!!");
    ...
    public float inputFloatLowerThan(String field, double high) {
        String res = inputWithOutEmpty(field);
        while (!isFloat(res) || Float.parseFloat(res) > high){
            if (!isFloat(res)) {
                System.out.println("Pls input number !!! Please input again");            
            } else {
                System.out.println("Must be lower than " + high + " !!!");
